...
// SoA (Structure of Arrays) 版 N-body (CPU ハイブリッド: MPI + OpenMP)
// AoS (struct Particle の配列) を廃止し、成分ごとの独立した連続配列で保持することで、
// ストライド1のメモリアクセスを実現し、キャッシュ効率と自動SIMD化 (SVE2 / AVX-512) を促進する。
...
int main(int argc, char** argv) {
  ...
  // __restrict__: ポインタ間にエイリアスがないことを示し、自動SIMD化を促進する
  double* __restrict__ mass = static_cast<double*>(std::malloc(sizeof(double) * N));
  double* __restrict__ x    = static_cast<double*>(std::malloc(sizeof(double) * N));
  double* __restrict__ y    = static_cast<double*>(std::malloc(sizeof(double) * N));
  double* __restrict__ z    = static_cast<double*>(std::malloc(sizeof(double) * N));
  double* __restrict__ vx   = static_cast<double*>(std::malloc(sizeof(double) * N));
  double* __restrict__ vy   = static_cast<double*>(std::malloc(sizeof(double) * N));
  double* __restrict__ vz   = static_cast<double*>(std::malloc(sizeof(double) * N));

  // 更新先（ローカル担当分）
  double* __restrict__ nx  = static_cast<double*>(std::malloc(sizeof(double) * local_particle_count));
  double* __restrict__ ny  = static_cast<double*>(std::malloc(sizeof(double) * local_particle_count));
  double* __restrict__ nz  = static_cast<double*>(std::malloc(sizeof(double) * local_particle_count));
  double* __restrict__ nvx = static_cast<double*>(std::malloc(sizeof(double) * local_particle_count));
  double* __restrict__ nvy = static_cast<double*>(std::malloc(sizeof(double) * local_particle_count));
  double* __restrict__ nvz = static_cast<double*>(std::malloc(sizeof(double) * local_particle_count));
  ...
  for (int run = 0; run < 1; ++run) {
    ...
    for (int step = 0; step < STEPS; ++step) {
      #pragma omp parallel for default(none) shared(x, y, z, vx, vy, vz, mass, nx, ny, nz, nvx, nvy, nvz, N, start_index, end_index, DT, G)
      for (int i = start_index; i < end_index; ++i) {
        double ax = 0.0;
        double ay = 0.0;
        double az = 0.0;

        // 自粒子の位置は j ループ不変のためレジスタにロード
        const double xi = x[i];
        const double yi = y[i];
        const double zi = z[i];

        // j ループは x[j] 等をストライド1で走査するため自動SIMD化されやすい
        for (int j = 0; j < N; ++j) {
          if (i == j) {
            continue;
          }
          ...
          // 1.0 / sqrt を明示的に作り、SVE2等の逆数平方根近似命令を有効化する
      ...
      // これにより、共有されている成分配列が全てのプロセスで更新される
	  }
