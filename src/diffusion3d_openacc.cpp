...
void diffusion3d(int rank, int nx, int ny, int nz, int mgn, float dx, float dy, float dz, float dt, float kappa, const float *f, float *fn) {
    ...
    #pragma acc parallel loop independent collapse(3) present(f[0:nx*ny*(nz+2*mgn)], fn[0:nx*ny*(nz+2*mgn)])
    for (int k = 0; k < nz; ++k) {
        for (int j = 0; j < ny; ++j) {
            for (int i = 0; i < nx; ++i) {
...
int main(int argc, char** argv) {
    ...
    #pragma acc data copy(f[0:ln]) create(fn[0:ln])
    {
        for (int step = 0; step < STEPS; ++step) {
            ...
            diffusion3d(rank, nx, ny, nz, mgn, dx, dy, dz, dt, kappa, f, fn);
