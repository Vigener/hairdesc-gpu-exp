...
### 2. SoA化による非合体アクセス解消と性能差の再拡大

- メモリへの非合体アクセス（Uncoalesced Access）を解消した **Structure of Arrays (SoA)** 構造への移行により、GPUの実行時間は 6.42秒 から **3.53秒** へと短縮（**1.82倍の高速化**）された。
- これにより、AoS時に一時 **4.3倍** まで縮小していた GPU/CPU（Grace）性能比は、再び **7.9倍** （Xeon MAXに対しては **14.2倍**）へと拡大した。
- `sqrt()` ボトルネック自体は依然として存在するものの、メモリアクセスの最適化によってH100 GPUの倍精度浮動小数点演算コアのポテンシャルが効率的に引き出された。

### 3. Grace CPU の卓越した並列スケーリング効率と GPU SoA のスケーラビリティ

- **Xeon MAX** (2ノード) のスケーリング効率は **81.0%** であり、`MPI_Allgather` 同期通信時に 112 スレッドの同期待けオーバーヘッドが顕在化していた。
- **Grace CPU** (2ノード) は、**97.5%** という AoS GPU (98.1%) とほぼ同等の極めて高い Strong Scaling 効率を達成した。
- **GPU (SoA)** 2ノードでは、成分配列ごとに Allgather を計6回実行するオーバーヘッドが加わるものの、実行時間 1.84秒（スケーリング効率 **95.8%**）と、依然として高い並列拡張性を維持している。
...
| **GPU** (H100 AoS)| 1,337       | **3,944** (3.94 TFLOPS)      | **5.9%**       |
| **GPU** (H100 SoA)| 2,431       | **7,171** (7.17 TFLOPS)      | **10.7%**      |

- この擬似 FLOPS を適用した場合、GPU (SoA) は H100 GPU の FP64 理論ピークに対し **10.7%** の実効ピーク効率に達する。Grace CPU の 50.6% に比べると利用率は依然として低いものの、AoS版（5.9%）からほぼ倍増しており、メモリアクセス最適化による演算器の効率向上が定量的に裏付けられた。
