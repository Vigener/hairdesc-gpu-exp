...
- **[4] プロファイラ予測値 (64.79%) と実測向上率 (81.8%) の乖離要因**
...
## 6. CPU SoA化への水平展開と対比実験

Nsight Compute による非合体アクセス解消（SoA化）の劇的な効果を踏まえ、CPU版（`src/nbody_hybrid_soa.cpp`）に対しても SoA 構造へのリファクタリングを水平展開し、同一の問題規模（N=65536, STEPS=100）での性能変化を計測した。

### A. 実測結果（Miyabi-G Grace CPU, 72スレッド）

| 構成        | AoS版時間 (GFLOPS) | SoA版時間 (GFLOPS)  | 向上率                   |
| :---------- | :----------------- | :------------------ | :----------------------- |
| **1ノード** | 27.79秒 (309.0)    | **25.70秒 (334.3)** | **1.08倍 (8.1%向上)** 🚀 |
| **2ノード** | 14.24秒 (603.0)    | **13.38秒 (641.9)** | **1.06倍 (6.4%向上)**    |

### B. アーキテクチャ的な要因分析（CPU vs GPU）

1. **CPUでの向上幅（8.1%）がGPU（81.8%）に比べて限定的な理由**:
   - **巨大なオンチップキャッシュとプリフェッチャ**:
     N=65536 の粒子データ（約 3.67 MB）は、Grace CPU の大容量L2キャッシュ（コアあたり1MB、計72MB）および共有L3キャッシュに完全に収まるサイズである。
     AoS版であっても、CPUの優秀なハードウェアプリフェッチャがメモリ階層から先行してデータをキャッシュへ読み込むことで、ロードレイテンシを大幅に隠蔽できていたため、データ構造の不連続性によるペナルティが相対的に小さかった。
   - **純粋な自動ベクトル化（SVE2）効率の向上**:
     SoA化により配列ポインタが連続（ストライド1）となり、`__restrict__` ポインタ修飾によるエイリアス排除が進んだ結果、コンパイラがより効率的なSIMD（SVE2）ロード命令を自動生成できるようになり、これが約8%の純粋な命令実行性能の差となって現れた。
2. **「レイテンシ隠蔽能力」の対比**:
...
さらに、この最適化を CPU へ水平展開することで、メモリ配置が Grace CPU の自動SIMD化（SVE2）にも有効に作用し、**1.08倍 (8.1%向上)** の追加高速化が得られた。

本フェーズの検証により、データ構造の合体化（SoA）が現代のプロセッサ最適化（GPU合体アクセスおよびCPUベクトルロード）において極めて普遍的かつ強力なアプローチであることが定量的に証明された。
