...
*(※画像配置プレースホルダー：ここに `images/ncu_summary_nbody_soa.png` を配置してください)*
...
### B. 実測ルーフラインモデルとスループット詳細

*(※画像配置プレースホルダー：ここに `images/ncu_details_nbody_soa.png` を配置してください)*
_図2：Nsight Compute による `main_143`（力学計算カーネル）の Roofline モデルと詳細スループット_

- **画像からの解説**:
  `Compute (SM) Throughput: 43.54%` および `Memory Throughput: 13.12%` を達成。倍精度演算ユニット（FP64）が最も活用されており、キャッシュ常駐効果によって動作プロット（オレンジの点）が算術強度約 24,000 FLOP/byte の演算律速領域（ルーフライン平坦部）に位置していることが実証されている。
